...
const NavBar = () => {
  ...
  // Effet pour récupérer les données utilisateur et paramètres
  useEffect(() => {
    const fetchParamsAndUser = async () => {
      ...
      try {
        ...
        if (token) {
          ...
          setAbonnementCount(activeAbonnements);
        } else {
          setIsAuthenticated(false); // Pas de token, utilisateur non connecté
        }
      } catch (error) {
        console.error("Erreur lors de la récupération des données:", error);
        setIsAuthenticated(false); // En cas d’erreur, considérer comme non connecté
      } finally {
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden lg:flex items-center space-x-1">
              ...
              return (
                <Link
                  ...
                  className={`relative px-3 py-2 text-sm font-medium transition-colors duration-200 rounded-md ${
                    isActive ? "text-emerald-600 bg-emerald-50" : "text-gray-700 hover:text-emerald-600 hover:bg-emerald-50/50"
                  }`}
                >
                  {link.name}
                  {isActive && (
            ...
            {!isLoading ? (
              ...
              <div className="w-8 h-8 rounded-full bg-gray-200 animate-pulse" /> // Placeholder pendant le chargement
            )}
          </div>
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 max-h-[70vh] overflow-y-auto">
              ...
              {navLinks.map((link) => {
                ...
                return (
                  ...
                  >
                    {link.name}
                    {link.path === "/admin" && <ShieldCheck className="ml-2 h-4 w-4 text-emerald-500" />}
                  </Link>
                );
              })}
