...
export const ModalContainer: React.FC<ModalContainerProps> = ({ isOpen, onClose, children, title, size = "md" }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative bg-[#F5F5F5] rounded-xl shadow-lg transform transition-all duration-300 w-full ${
          sizeClasses[size]
      ...
      >
        ...
        {title && (
          <div className="flex items-center justify-between p-4 border-b border-[#F5E8C7] shrink-0">
        ...
        <div className="p-6 overflow-y-auto scrollbar-thin scrollbar-thumb-[#D2B48C] scrollbar-track-[#F5E8C7] flex-1">
          {children}
        </div>
      </div>
    </div>
